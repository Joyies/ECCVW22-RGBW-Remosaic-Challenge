...
# import sys
# sys.path.append('/home/notebook/data/group/wurongyuan/code/KAIR-master/models')
...
class ResBlock(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        ...
        self.self_conv = nn.Sequential(
                nn.Conv2d(in_channels, in_channels*2, kernel_size=1), 
                nn.BatchNorm2d(in_channels*2)
            )
        self.mlp = Mlp(in_channels, in_channels*2)
        self.win = 8#win
        self.shifts = False#shifts
        self.beta1 = nn.parameter.Parameter(data=torch.rand((1)), requires_grad=True)
        self.beta2 = nn.parameter.Parameter(data=torch.rand((1)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        x1 = self.self_conv(F.interpolate(self.relu(res),[h-h%self.win,w-w%self.win]))

        q, v = rearrange(
                    x1, 'b (qv c) (h dh) (w dw) -> qv (b h w) (dh dw) c', 
                    qv=2, dh=self.win, dw=self.win
                )
        ...
        y_ = rearrange(
            ...
            h=h//self.win, w=w//self.win, dh=self.win, dw=self.win
        )

        y_ = F.interpolate(y_,[h,w])
        
        return self.beta1 * y_ + res + x
